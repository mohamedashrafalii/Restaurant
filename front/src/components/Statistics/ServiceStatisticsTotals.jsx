...
class ServiceStatisticsTotals extends Component {
  ...
  totalSpendings = async  ()=> {
     ...
     await axios.get(
      staticVariables.backendUrl+"/stats/totalSpendings3?startDate="+this.state.newOrnek.startDate+"&endDate="
      +this.state.newOrnek.endDate+"&userId="+this.state.userId+"&subDepartment="+this.state.subDepartment
      ,{headers: { authToken : localStorage.getItem("token") }}
      ) .then((res) =>
      {  
        console.log(res.data.sums)
    this.setState({ sums: res.data.sums,
...
render=()=>{
  ...
  // if(this.props.value)
  // {
    let  ornek = this.state.ornek?this.state.sums.map((target) => {
        return (
                ...
                <tr key={target.id}>
                 ...
                 <td style={{color:"#000"}}>{target[0]}</td>
                 <td style={{color:"#000"}}>{target[1]}</td>
...
export default ServiceStatisticsTotals
